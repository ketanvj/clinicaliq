...
data/ingest.py
--------------
Ingests Apollo Health Clinic documents into ChromaDB.
...
Why are prices NOT in these documents?
  Prices are in data/clinic_data.db (see seed.py). Putting "consultation: Rs. 500"
  in a document would create two sources of truth. When the price changes (seed.py
  is re-run), the document would still show the old figure while the database has
  the new one. The compliance node specifically checks that ClinicalIQ never quotes
  prices from documents -- prices must always come from a database tool call.
...
def main() -> None:
    ...
    print("\nSetup complete. Run 'python s01/solution/main.py' to start ClinicalIQ.")
